<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" type="primary" @click="handleCreate()">新建Client</el-button>
    ...
    >
      ...
      <el-table-column label="Client IP" min-width="200" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.ip }}</span>
        </template>
      </el-table-column>
      <el-table-column label="Client 端口" min-width="200" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="textMap[dialogStatus]" width="600px">
      <el-form ref="dataForm" :rules="rules" :model="nodeModel" label-position="left" label-width="120px" style="width: 400px; margin-left:30px;">
        ...
        <el-form-item label="Client 名称" prop="name">
          <el-input v-model="nodeModel.name" />
        </el-form-item>
        <el-form-item label="Client IP" prop="ip">
          <el-input v-model="nodeModel.ip" />
        </el-form-item>
        <el-form-item label="Client 端口" prop="port">
          <el-input v-model="nodeModel.port" placeholder="8081" type="number" />
        </el-form-item>
      </el-form>
    ...
    <el-dialog :visible.sync="dialogInstances" title="instance 列表" width="800px">
      <div class="filter-container">
        <el-button class="filter-item" type="info" @click="getInstances()">刷新列表</el-button>
      </div>
      <el-table
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: 'Client 名称不能为空', trigger: 'change' }],
        ip: [{ required: true, message: 'Client IP不能为空', trigger: 'change' }],
        port: [{ required: true, message: 'Client 端口不能为空', trigger: 'change' }]
      },
      dialogStatus: 'create'
    }
  },
  ...
  methods: {
    fetchData() {
      ...
      getNodeClients(this.listQuery).then(res => {
    ...
    getInstances() {
      ...
      getInstances(this.serverIdTmp).then(res => {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (this.dialogStatus === 'create') {
            addNodeClient(this.nodeModel).then(res => {
              this.operationRes(res)
            })
          }
          if (this.dialogStatus === 'update') {
            updateNodeClient(this.nodeModel).then(res => {
              this.operationRes(res)
            })
          }
        }
    ...
    handleDelete(row) {
      this.$confirm('删除Client信息会导致节点服务停止', '确定删除Client信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteNodeClient(row.id).then((res) => {
          if (res.data === 'success') {
            this.fetchData()
            this.$message({
              message: '删除成功',
    ...
    handleLog(row) {
      this.$router.push('nodeClient/log?id=' + row.id)
    },
    handleStartInstance(row) {
      ...
      this.$confirm('启动Instance服务', '确定启动Instance服务', {
        ...
        startInstance(row.name, this.serverIdTmp).then((res) => {
          if (res.data) {
            this.getInstances()
            this.$message({
    ...
    handleStopInstance(row) {
      ...
      this.$confirm('集群模式下停止实例其它主机将会抢占执行该实例', '停止 Instance 服务', {
        ...
        stopInstance(row.name, this.serverIdTmp).then((res) => {
          if (res.data) {
            this.getInstances()
            this.$message({
